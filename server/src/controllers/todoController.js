...
const todoController = {
  ...
  async createTodo(req, res) {
    try {
      const userId = getUserId(req);
      const { todo, due, due_time, type, category, goal, priority, status, description } = req.body;
      
      if (!todo || !todo.trim()) {
        return res.status(400).json({ error: 'Todo text is required' });
      }
      
      const newTodo = await Task.create(userId, { todo, due, due_time, type, category, goal, priority, status, description });
  ...
  async updateTodo(req, res) {
    try {
      const userId = getUserId(req);
      const { index } = req.params; // This is actually the task ID now
      const { todo, due, due_time, type, category, goal, priority, status, description } = req.body;
