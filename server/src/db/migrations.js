...
async function initializeDatabase() {
  try {
    ...
    // Add missing columns to existing tasks table (for migrations)
    await pool.query(`
      DO $$ 
      BEGIN
        ...
        -- Rename start_time to due_time for tasks (migration)
        IF EXISTS (
          SELECT 1 FROM information_schema.columns 
          ...
          ALTER TABLE tasks RENAME COLUMN start_time TO due_time;
        END IF;
        
        -- Remove end_time column if it exists
        IF EXISTS (
          SELECT 1 FROM information_schema.columns 
    ...
    // Add missing columns to existing events table (for migrations)
    await pool.query(`
      DO $$ 
      BEGIN
    ...
    // Add missing columns to existing goals table (for migrations)
    await pool.query(`
      DO $$ 
      BEGIN
        ...
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns 
          WHERE table_name = 'goals' AND column_name = 'target'
        ) THEN
          ALTER TABLE goals ADD COLUMN target VARCHAR(255);
        END IF;
